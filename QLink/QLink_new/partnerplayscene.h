#pragma once
#ifndef PARTNERPLAYSCENE_H
#define PARTNERPLAYSCENE_H
...
class PartnerPlayScene : public QWidget
{
    Q_OBJECT
public:
    explicit PartnerPlayScene(QWidget *parent = nullptr);
    int maxX = 22;
    int maxY = 11;
    //存储各方块情况的数组
    // 如果值为0 表示空 即未有占用
    ...
    void setPlayBtn();
    //更新玩家位置
    void updatePlayerLocation(Player *playerTmp);
    //重写键盘按下函数
    void keyPressEvent(QKeyEvent *event);
    //处理锁定目标函数
    void solveChecked(int i,int x,int y,Player *playerTmp);
...
signals:
    ...
    //在点击退出按钮时触发信号给上一界面
    void partnerPlayClose();
};

#endif // PARTNERPLAYSCENE_H
